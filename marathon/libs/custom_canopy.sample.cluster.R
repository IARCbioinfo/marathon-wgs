custom_canopy.sample.cluster = function (R, X, sna_cluster, WM, Wm, epsilonM, epsilonm, C = NULL, 
          ...
          cell.line = NULL, plot.likelihood = NULL) 
{
  ...
  # }
  if (plot.likelihood) {
    pdf(file = paste(projectname, "_likelihood.pdf", sep = ""), 
        width = 10, height = 5)
  }
  sampname = colnames(R)
  sna.name = rownames(R)
  ...
  for (k in K) {
    ...
    for (numi in 1:numchain) {
      cat("\tRunning chain", numi, "out of", numchain, 
          "...\n")
      text = paste(paste(paste(paste("(", 1:(k - 1), ",", 
                                     sep = ""), collapse = ""), k, sep = ""), paste(rep(")", 
                                                                                        (k - 1)), collapse = ""), ";", sep = "")
      runif.temp = runif(1)
      ...
      else if (k == 7 & runif.temp > 1/4 & runif.temp <= 
               2/4) {
        text = c("(1,((2,3),(4,(5,(6,7)))));")
      }
      else if (k == 7 & runif.temp > 2/4 & runif.temp <= 
               3/4) {
        text = c("(1,((2,3),((4,5),(6,7))));")
      }
      ...
      tree$sna.cluster = initialsna(tree, paste("cluster", 
                                                unique(sna_cluster), sep = ""))
      sna.mat = cbind(sna = 1:nrow(R), (tree$sna.cluster)[sna_cluster, 
                                                          2:3])
      ...
      tree$likelihood = getlikelihood(tree, R, X, WM, 
                                      Wm, epsilonM, epsilonm)
      sampi = 1
      writei = 1
      ...
      while (sampi <= max.simrun) {
        if (sampi <= min.simrun) {
          ...
          sna.mat = cbind(sna = 1:nrow(R), (tree.new$sna.cluster)[sna_cluster, 
                                                                  2:3])
          colnames(sna.mat) = c("sna", "sna.st.node", 
                                "sna.ed.node")
          rownames(sna.mat) = sna.name
          ...
          tree.new$likelihood = getlikelihood(tree.new, 
                                              R, X, WM, Wm, epsilonM, epsilonm)
          ...
          samptree.accept.rate[sampi] = mean(samptree.accept[max(1, 
                                                                 sampi - 999):sampi])
          sampi = sampi + 1
        }
        else {
          ...
          samptree.accept.rate[sampi] = mean(samptree.accept[max(1, 
                                                                 sampi - 999):sampi])
          if ((sampi >= 2 * min.simrun) & (samptree.lik[sampi] <= 
                                           mean(samptree.lik[max((sampi - 1000), 1):max((sampi - 
                                                                                         1), 1)])) & (samptree.accept.rate[sampi] <= 
                                                                                                      mean(samptree.accept.rate[max((sampi - 1000), 
                                                                                                                                    1):max((sampi - 1), 1)])) & (samptree.accept.rate[sampi] <= 
                                                                                                                                                                 0.1)) 
            break
          sampi = sampi + 1
        }
    ...
    if (plot.likelihood) {
      ...
      plot(sampchaink.lik[[1]], xlim = c(1, max(xmax)), 
           ylim = c(min(ymin), max(ymax)), xlab = "Iteration", 
           ylab = "Log-likelihood", type = "l", main = paste("Post. likelihood:", 
                                                             k, "branches"))
      for (numi in 2:numchain) {
        points(sampchaink.lik[[numi]], xlim = c(1, max(xmax)), 
               ylim = c(min(ymin), max(ymax)), col = numi, 
               type = "l")
      }
      plot(sampchaink.accept.rate[[1]], ylim = c(0, 1), 
           xlim = c(1, max(xmax)), xlab = "Iteration", 
           ylab = "Acceptance rate", type = "l", main = paste("Acceptance rate:", 
                                                              k, "branches"))
      for (numi in 2:numchain) {
        points(sampchaink.accept.rate[[numi]], ylim = c(0, 
                                                        1), xlim = c(1, max(xmax)), col = numi, type = "l")
      }
      par(mfrow = c(1, 1))
    }
